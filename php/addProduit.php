    ...
    if (isset($_POST['send'])){
        if ($_SESSION['admin'] == true)
        {
            if (!empty($_POST['title']) AND !empty($_POST['price']) AND !empty($_POST['image']) AND !empty($_POST['description']) AND !empty($_POST['cate']))
            {
                ...
                header('Location: ../admin/admin.php?valideraddprod='.$valideraddprod.'');
            }
            else { 
            	$erreurcate = "Oupss vous n'avez pas saisie un titre !";
                header('Location: ../admin/admin.php?erreuraddprod='.$erreuraddprod.'');
            }
